...
docker run -v "${output_dir}:/output_dir" -v /tmp:/tmp "pdok/ngr-services-spider:latest" layers $nr_svc_flag --snake-case --pretty -s /tmp/sorting-rules.json -m flat -p "OGC:WMS,OGC:WFS,OGC:WCS,OGC:WMTS,OGC:API tiles,OGC:API features" "/output_dir/${output_file_base}" --jq-filter '.layers[] |= with_entries(
