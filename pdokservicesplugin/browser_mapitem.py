...
class MapDataItem(QgsDataItem):
    def __init__(self, parent, name, dataset, editable=False):
        QgsDataItem.__init__(
            self, QgsDataItem.Custom, parent, name, "/MapTiler/maps/" + name
        )
    ...
    def handleDoubleClick(self):
        ...
        if "custom" in self._dataset:
            ...
            utils.is_qgs_vectortile_api_enable()
            and prefervector
            and "vector" in self._dataset
        ):
    ...
    def actions(self, parent):
        ...
        if "custom" in self._dataset:
            ...
            add_custom_action.triggered.connect(lambda: self._add_custom_to_canvas())
        ...
        # MapTiler map
            ...
            add_raster_action.triggered.connect(lambda: self._add_raster_to_canvas())
    ...
    def _add_raster_dem_to_canvas(self, data_key="raster-dem"):
        ...
        try:
            ...
            (
                ...
                max_ramp_value,
                color_ramp,
            ) = utils.load_color_ramp_from_file(COLOR_RAMP_PATH)
    ...
    def _add_vtlayer_from_style_json(
        self,
        ...
        ordered_sources = {
            k: v for k, v in sorted(sources.items(), key=lambda item: item[1]["order"])
        }
        for source_id, source_data in ordered_sources.items():
            ...
            if source_data["type"] == "vector":
                ...
                if (
                    source_data.get("name") == "Terrain RGB"
                    and "https://api.maptiler.com/tiles/terrain-rgb" in zxy_url
                ):
                    uri = (
                        f"zmin=0&zmax=12&type=xyz"
                ...
                for rlayer_json in rlayers:
                    ...
                    (
                        styled_renderer,
                        styled_resampler,
                    ) = converter.get_raster_renderer_resampler(renderer, rlayer_json)
        ...
        if bool(candidate_warnings):
            ...
            for candidate in candidate_warnings:
                ...
                if (
                    "Could not retrieve sprite ''" in candidate
                    or "Could not retrieve sprite ' '" in candidate
                ):
                    continue
    ...
    def _add_vtlayer_from_tile_json(
        self,
        tile_json_data: dict,
        target_node: QgsLayerTreeGroup,
        attribution_text: str,
        ...
        try:
            ...
            if self._is_vector_json(json_url):
                if utils.is_qgs_vectortile_api_enable():
                    ...
                    widget = iface.messageBar().createMessage(
                        f"'{self.name()} Layer Loading Error',"
